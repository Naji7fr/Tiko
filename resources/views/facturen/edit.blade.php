...
<div class="page-header">
    <div class="container">
        <h1><i class="fas fa-file-invoice-dollar me-3"></i>Factuur Bewerken</h1>
        <p class="mb-0 mt-2 opacity-90">Bewerk de details van deze factuur</p>
    </div>
</div>
                    ...
                    <form action="{{ route('facturen.update', $invoice->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                <option value="openstaand" {{ old('status', $invoice->status) == 'openstaand' ? 'selected' : '' }}>Openstaand</option>
                                <option value="betaald" {{ old('status', $invoice->status) == 'betaald' ? 'selected' : '' }}>Betaald</option>
                                <option value="vervallen" {{ old('status', $invoice->status) == 'vervallen' ? 'selected' : '' }}>Vervallen</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Beschrijving van de factuur...">{{ old('description', $invoice->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>Factuur Bijwerken
                            </button>
