...
<div class="page-header">
    <div class="container">
        <h1><i class="fas fa-box-open me-3"></i>Product toevoegen</h1>
    </div>
</div>
...
<div class="container py-4">
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('producten.store') }}" novalidate>
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select id="categorie_id" name="categorie_id" class="form-select @error('categorie_id') is-invalid @enderror" required>
                        ...
                        @foreach($categorieen as $categorie)
                            <option value="{{ $categorie->id }}" @selected(old('categorie_id') == $categorie->id)>{{ $categorie->naam }}</option>
                        @endforeach
                    </select>
                    @error('categorie_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <select id="leverancier_id" name="leverancier_id" class="form-select @error('leverancier_id') is-invalid @enderror" required>
                        ...
                        @foreach($leveranciers as $leverancier)
                            <option value="{{ $leverancier->id }}" @selected(old('leverancier_id') == $leverancier->id)>{{ $leverancier->bedrijfsnaam }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea id="beschrijving" name="beschrijving" rows="3" class="form-control">{{ old('beschrijving') }}</textarea>
                </div>
                ...
                <button type="submit" class="btn btn-primary"><i class="fas fa-plus me-2"></i>Product toevoegen</button>
